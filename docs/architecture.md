...
- `EXPECTED_API_VERSION` (`"1.2.0"`) — sent on every backend call; the backend compares **major.minor** exactly (patch ignored). Consumers read it from core, never hardcode.
- `CONFIG_VERSION` (`1`).
